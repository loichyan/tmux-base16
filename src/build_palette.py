...
def build_palette_entries(config: SectionProxy):
    result: list[str] = []
...
def build_palette(path: str) -> str:
    ...
    assert config.has_section("dark"), "dark palette is missing"
    dark = config["dark"]
    light = config["light"] if config.has_section("light") else dark

    result += ['%if "#{!=:#{@base16-background},light}"']
    result += map(lambda s: "\t" + s, build_palette_entries(dark))
    result += ["%else"]
    result += map(lambda s: "\t" + s, build_palette_entries(light))
    result += ["%endif"]
