...
def build_palette_variant(config: SectionProxy):
    ...
    # Bright/Dimmed colors
    for variant, name in [("bright", "Bright"), ("dimmed", "Dimmed")]:
        d = config.get(variant)
        if d is None:
            continue
...
def build_palette(path: str) -> str:
    ...
    with open(path) as f:
        config.read_file(f.readlines())
    result = ["# This file is @generated by tmux-base16"]

    assert config.has_section("dark"), "dark palette is required"
    dark = config["dark"]
    light = config["light"] if config.has_section("light") else dark

    result += ['%if "#{!=:#{@base16-background},light}"']
    result += map(lambda s: "\t" + s, build_palette_variant(dark))
    result += ["%else"]
    result += map(lambda s: "\t" + s, build_palette_variant(light))
    result += ["%endif"]
